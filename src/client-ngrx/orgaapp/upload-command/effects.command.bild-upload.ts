...
import * as ac from './actions.command.bild-upload';
...
@Injectable()
export class OrgaappBildUploadEffects {

    @Effect()
        BildUpload$ = this.actions$.pipe(
            ofType(ac.POST_BILD_UPLOAD),
            map((x: ac.PostBildUploadAction) => {
                return this.ngrxManagerService.checkRequestCall(ac.POST_BILD_UPLOAD, x, RequestMethod.COMMAND, RequestType.Anfrage);
            }),
